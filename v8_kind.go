...
func (k Kind) mask() kinds { return kinds(1 << k) }

type kinds uint64

// if kNumKinds > 64, then this will fail at compile time.
const compileCheckThatNumKindsBitsFitInKindType = kinds(1 << kNumKinds)

func (mask kinds) Is(k Kind) bool {
	return (mask & k.mask()) != 0
}

func (mask kinds) String() string {
	var res []string
